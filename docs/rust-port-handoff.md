...
Claude（レビュー担当、ドキュメント・タスクファイル編集）と agy（実装担当）は**同一の作業ディレクトリ・同一のgitワークツリーを共有している**。両者が同時にコミット・ブランチ切り替え・ファイル編集を行うと、片方の変更が意図しないブランチに混入したり、`git checkout`で他方の未コミット変更を巻き込んだりする事故が起きうる(実際に2026-09-22、Claudeのタスクファイルコミットがagyの作業ブランチに混入し、agyの未コミットWIPを`git stash`で退避・復元する事態が発生した)。

- **Claudeがdocs/タスクファイルをコミットする前に、必ず`git status`と`git branch --show-current`を確認し、agyが別ブランチで作業中でないか(未コミット変更が無いか)を確かめること。**
- agyの未コミット変更を発見した場合、安易に`git checkout`/`git reset`等の破壊的操作を行わず、`git stash push -m "<説明>"`で退避してから作業し、完了後に復元すること(ただし複数の連続した小さな差分が同一ファイルに対して段階的に退避されるケースでは、後の差分が前の差分を包含するとは限らない——実際に「importの追加」と「テスト関数の追加」が別々の差分として退避され、両方が必要だったケースがあった。中身を確認せず「最新のものだけ採用」と判断しないこと)。
- 可能であれば、Claudeがdocs編集・マージ作業を行うタイミングと、agyが実装作業を行うタイミングを重ねないよう、ユーザー側で調整することが望ましい。
...
- **CCD参照構造による水素付加（`RUST_PORT_SPEC.md` §3.16、計画済み・未着手、2026-09-24）**:
  - X線構造など水素座標を持たない入力に対し、CCDテンプレートDB（§3.9〜3.11）を座標付きに拡張して`Superposer`剛体フィットで水素を付加する機能。PR#37〜40として4分割済み。`docs/tasks/TASK_ccd-hydrogenation.md`参照。
