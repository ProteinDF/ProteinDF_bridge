...
### PR#35: `Bond::setup()`のスケーラビリティ改善(完了 2026-09-19)

背景: 上記「着手前確認事項4」の通り、現状は全原子ペアの総当たり(O(n²))。YUIの目標(最大約100万原子)ではこのままでは実用に耐えない。`docs/tasks/PR35_PROPOSAL.md` にてClaudeに着手前プロポーザルを提示・承認を得て実装した。

採用アプローチ:
- `spatial.rs` を新設し、一様セルリスト `CellList` (half-neighborhood 13方向探索)を実装。
- 外部クエリAPIとして `CellList::for_each_neighbor_pair` を提供(インデックス順序規約 `i < j` を保証)。
- `Bond::setup()` で動的セルサイズ (`(2 * max_vdw + 0.4).max(3.0)`) による O(N) 探索を実施。
- `MAX_DENSE_MATRIX_ATOMS = 2000` を公開定数として定義し、2,000原子超の場合は dense 行列 (`distmat`/`bondmat`) のアロケーションをスキップして `None` を設定(メモリ枯渇を防止)。

完了の定義(実績):
1. 既存の `test_bond_setup`/`test_bond_setup_empty` を含む既存全テストがパス。
2. 3,150原子(15×15×14グリッド)の合成構造において、旧O(n²)実装とセルリスト新実装の結合ペア集合が**完全に一致**することをテストで検証済み (`test_bond_setup_cell_list_equivalence`)。
3. 100万原子規模の性能ベンチマーク (`test_benchmark_1m_atoms`) 測定結果:
   - **原子数**: 1,000,000原子 (100×100×100 グリッド、間隔 2.0 Å)
   - **原子生成時間**: 308.94 ms
   - **`Bond::setup()` 実行時間**: **3.39 秒** (リリースビルド)
   - **検出結合数**: 12,731,796 本
   - **メモリ使用量(概算)**: 従来の O(n²) dense 行列(4 TB 必要)を回避。座標配列自体は約24 MB(1,000,000原子 × 3 f64)で、セルリストのグリッドを含めても数十 MB 程度に収まると見積もられる(実測値ではなく概算)。
4. `cargo clippy --workspace --all-targets -- -D warnings` および `cargo fmt --check` パス。
...
- **ブランチ運用ルール(MUST項目)を厳守**: `feature/phase10-prM`ブランチを`develop`から切って作業し、`develop`へは自分でマージせず、レビュー承認を待つ。PR#27(依存なし、最も軽い)→PR#28→PR#29→PR#30→(PR#31〜33は並行可)→PR#34(ドキュメントのみ)→PR#35→PR#36(ドキュメントのみ、Phase 10最後)の順に着手すること。
