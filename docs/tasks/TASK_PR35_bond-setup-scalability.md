# TASK_PR35+: `Bond::setup()`のスケーラビリティ改善

> 本タスクは `docs/rust-port-handoff.md` の「Phase 10」節(RUST_PORT_SPEC.md §9対応)から抽出したものです。全体の背景・優先順位・他PRとの関係は同ドキュメントを参照してください。番号が「35+」なのは、設計の都合で複数PRに分割する可能性があるため(例: PR#35でコア実装、PR#36でベンチマーク、等)。分割する場合はブランチ名を `feature/phase10-pr35`、`feature/phase10-pr36` のように連番にすること。
>
> (2026-09-19付記: 元々は「PR#34+」として計画されていましたが、TASK_PR31〜33完了後に必要になったドキュメントのみの小タスク(ファイル由来結合とVDWヒューリスティックの優先順位確立)を`TASK_PR34_bond-priority-policy-doc.md`としてPR#34に割り当てたため、本タスクは1つ繰り下げてPR#35+としました。)

## ブランチ運用(MUST)

- `develop` から `feature/phase10-pr35`(分割する場合は以降 `pr36`, `pr37`...)を作成して作業する。
- **`develop` へは自分でマージしない。** 実装・テスト・`cargo clippy`/`cargo fmt`確認が終わったら、ブランチ名と完了内容をユーザー経由でClaudeに報告し、レビュー承認(または修正依頼)を待つこと。
- 依存関係: なし(TASK_PR31〜34のファイル由来結合・ドキュメント対応とは独立に実装可能)。Phase 10の中で最も設計判断を伴う大きめのタスクなので、**着手前に方針をPR説明(または着手前のコメント)でClaudeに確認すること。**
