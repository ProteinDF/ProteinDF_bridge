...
### 3.12 結合解決の統合エントリポイント(ファイル由来 > CCDテンプレート > ヒューリスティック、計画中、未着手)

#### 背景

§3.8で確立した優先順位(ファイル由来結合 > CCDテンプレート > `Bond::setup()`ヒューリスティック)は、現状**呼び出し側が正しい順序で複数のAPIを個別に呼ぶ責任を負っている**(`apply_ccd_bond_templates()` → `Bond::setup()`)。この2段階呼び出しを1回の呼び出しに統合し、`proteindf-bridge`を使うプログラム側の実装を簡単にしたい、というユーザーからの要望。

`Bond::setup()`自体を「CCDテンプレートも内部で使う」ように変更することは**推奨しない**。理由:
- `Bond::setup()`は現状「純粋なVDW/共有結合半径ヒューリスティックのみ」を行う関数として明確にドキュメント化・テストされている(§3.9フェーズAの`test_apply_ccd_bond_templates_1hls_real_pdb`は「`Bond::setup()`単独では全結合が次数1になる」ことをベースラインとして検証しており、これが壊れる)。
- `bond.rs`はPhase 1由来の基礎プリミティブであり、Phase 10以降に追加された高レベル機能(CCDテンプレートDB、mmCIF連携)に依存させるのはレイヤリング上望ましくない。

代わりに、**両方を正しい順序で組み合わせる新しい便利関数を追加する。**

**前提条件**: 本タスクは`fix/bond-setup-duplicate-bonds`(`Bond::setup()`が既存結合を重複登録するバグの修正)の完了・マージを前提とする。この修正がないと、CCDテンプレート適用後にヒューリスティックを実行した際に重複した結合レコードが生成されてしまう。

#### 対象

1. `AtomGroup`に統合エントリポイント(例: `AtomGroup::resolve_bonds(&mut self, db: &CcdTemplateDb) -> Result<()>`)を追加する。内部で以下を順に行う:
   1. (既にファイル由来の結合がある場合はそのまま尊重、上書きしない — 既存の`apply_ccd_bond_templates`/`Bond::setup()`双方の「既存結合を上書きしない」実装を前提とする)
   2. `apply_ccd_bond_templates(db)`を呼ぶ(CCDテンプレートで補完)。
   3. `Bond::setup()`を呼ぶ(修正済みのため、ステップ2で登録済みの結合は重複せず、残りの原子ペアのみヒューリスティックで補完される)。
2. `db`引数を受け取ることで、呼び出し側は組み込みデフォルトDB(`CcdTemplateDb::global()`)をそのまま渡すことも、§3.11で追加した拡張DBを渡すこともできるようにする(呼び出しの単純化と拡張性を両立)。
3. `RUST_PORT_SPEC.md` §3.8の「呼び出し側の推奨利用パターン」を、この新しいエントリポイントを使う形に更新する。

#### 完了の定義(想定)

1. 実PDBフィクスチャ(`1hls.pdb`)で、`resolve_bonds()`を1回呼ぶだけで、CCDテンプレートによる結合次数(GLU側鎖の二重結合等)とヒューリスティックによる補完(ペプチド結合等)の両方が、重複なく正しく得られることを検証する回帰テストを追加する。
2. `Bond::setup()`単体・`apply_ccd_bond_templates()`単体は、それぞれ独立して呼んでも既存の(§3.9フェーズA・§3.10の)テストが引き続きパスすることを確認する(既存APIの意味を変えていないことの回帰確認)。
